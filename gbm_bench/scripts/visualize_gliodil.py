...
if __name__ == "__main__":
    # Example:
    # python scripts/visualize_gliodil.py
    ...
    for patient_ind, patient in enumerate(gliodil.patients):
        print(f"Visualizing {patient_ind}/{len(gliodil.patients)}...")
        
        patient_identifier = patient["patient_id"]
        exam_dir_preopop = gliodil.get_patient_exams(patient_id=patient_identifier, timepoint="preop")[0]["t1"].parent
        exam_dir_followup = gliodil.get_patient_exams(patient_id=patient_identifier, timepoint="followup")[0]["t1"].parent
