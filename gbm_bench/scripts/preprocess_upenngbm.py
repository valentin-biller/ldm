...
if __name__ == "__main__":
    ...
    # nohup python -u scripts/preprocess_upenngbm.py -cuda_device 0 > tmp_upenn_preproc.out 2>&1 &
    ...
    for patient_ind, patient in enumerate(upenn_gbm.patients):
        ...
        preop_exam = upenn_gbm.get_patient_exams(patient_id=patient_id, timepoint="preop")[0]  # Find first preop exam
        preop_exam_dir = preop_exam["t1"].parent

        # Loop through followup exams
        followup_exams = upenn_gbm.get_patient_exams(patient_id=patient_id, timepoint="followup")
