...
if __name__ == "__main__":
    ...
    for patient_ind, patient in enumerate(lumiere.patients):
        print(f"Visualizing {patient_ind}/{len(lumiere.patients)}...")
        
        patient_identifier = patient["patient_id"]
        exam_dir_preopop = lumiere.get_patient_exams(patient_id=patient_identifier, timepoint="preop")[0]["t1c"].parent
        exam_dir_followup = lumiere.get_patient_exams(patient_id=patient_identifier, timepoint="followup")[0]["t1c"].parent
